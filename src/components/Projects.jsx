...
const Projects = () => {
  return (
    <div
      className="w-full rounded-3xl container   p-4   border-2 text-stone-200 border-white  m-auto py-8  px-8 sm:px-8 md:px-10 lg:px-12"
      id="projects"
    >
      ...
      <div className="grid  grid-cols-1  gap-6   sm:gap-4   px-0 sm:px-2 md:px-4 lg:px-6  ">
        {array.map((arr) => {
          return <ProjectCompo data={arr} key={arr.id} />;
        })}
      </div>
    </div>
  );
